...
# call this function and then set the output as the input of the prediction model
def fmuinitialize():
    # define the model name and simulation parameters/
    fmu_filename = "c:/Users/adminihb/Documents/GitHub/GunnerusDigitalTwin/apps/api/models/model1/PMAzimuth.fmu"
    unzipDirection = "c:/Users/adminihb/Documents/GitHub/GunnerusDigitalTwin/apps/api/models/model1/"
    ...
    # extract the FMU
    unzipdir = extract(fmu_filename,unzipdir=unzipDirection)

    print(model_description.guid)
    print(unzipdir)
    print(model_description.coSimulation.modelIdentifier)
...
class ModelPredict:
    ...
    def ship(self, x, F):
        ...
        C_A = np.array(
            [
                [0, 0, Yvdot * x[4][0] + 1 / 2 * (Yrdot * x[5][0] + Nvdot * x[5][0])],
                [0, 0, -Xudot * x[3][0]],
                [
                    -Yvdot * x[4][0] - 1 / 2 * (Yrdot * x[5][0] + Nvdot * x[5][0]),
                    Xudot * x[3][0],
                    0,
                ],
            ]
    ...
    def hydrodynamic(self, u, v, r):
        ...
        Y_coeff = np.array(
            [1093539, -185048, 4177435, -29940, 723602, 1570919, -16493927, -226243]
        )
        N_coeff = np.array(
            [-8614233, -508092, -2180055, -70853, -3434695, -1307083, 7020848, -3912253]
        )
        X_h = -(
            2.6528 * u**4 - 58.026 * u**3 + 480.12 * u**2 - 1756.2 * u + 2400.8
        ) * 1000 + X_coeff.dot(np.array([v**2, u * (v**2), u * v * r, v * r]))
    ...
    def get_one_side_f(self, ps, time, step_size, x0, rudder_cmd, rpm_cmd):
        ...
        vr_inputs = [
            self.vrs["input_act_angle"],
            self.vrs["input_act_revs"],
        ]  # deg and rpm
        vr_outputs4 = [
            self.vrs["output_force_surge"],
            self.vrs["output_force_sway"],
            self.vrs["output_torque"],
        ]
        parameter_blinds = [
            "input_x_rel_ap",
            "input_z_rel_bl",
            "input_prop_diam",
            "input_distancetohull",
            "input_bilgeradius",
            "input_rho",
            "input_lpp",
        ]
        ...
        self.fmu.setReal(parameter_reference, [0.0, 0.55, 1.9, 1.5, 3.0, 1025.0, 33.9])
        self.fmu.setReal([self.vrs["input_y_rel_cl"]], [ly])

        self.fmu.setReal([self.vrs["input_cg_x_rel_ap"]], [15.52])
        self.fmu.setReal([self.vrs["input_cg_y_rel_cl"]], [0.0])
        ...
        self.fmu.setReal(
            [self.vrs["input_yaw_vel"]], [x0[5] * 180 / 3.14]
        )  # input_yaw_vel [deg/s]

        self.fmu.doStep(currentCommunicationPoint=time, communicationStepSize=step_size)
        val_outputs = {
            "output_force_surge": self.fmu.getReal([vr_outputs4[0]])[0],
    ...
    def simulate(self):
        ...
        ini_s = np.array(
            [self.x0, self.y0, self.psi0, self.u0, self.v0, self.r0]
        ).reshape(-1, 1)

        val_cmd = {
            "pt_rpm": self.rpm_cd_pt,
            "pt_azi": self.rudder_cd_pt,
            "sb_rpm": self.rpm_cd_sb,
        ...
        while time < self.delta_t:
            F_pt = self.get_one_side_f(
                "P", time, 1.0, x0, rudder_cmd=self.rudder_cd_pt, rpm_cmd=self.rpm_cd_pt
            )
            F_sb = self.get_one_side_f(
                "S", time, 1.0, x0, rudder_cmd=self.rudder_cd_sb, rpm_cmd=self.rpm_cd_sb
            )
            ...
            rowsDict = self.Merge(
                self.Merge(self.Merge(val_time, val_state), val_cmd), val_ini_s
            )
