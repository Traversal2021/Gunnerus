...
async function getiss() {
  ...
  for (var i = 0; i < Object.keys(plot_data).length; i++) {
    plot_data[topic_list[i]][data[topic_list[i]][1]] = data[topic_list[i]][0];
  }

  UpdateGraph(plot_data);

  double_link1_data = data.double_link1[0];
  double_link2_data = -data.double_link2[0];
  yaw_support_data = data.yaw_support[0];
  if (yaw_support_data == "None") {
    yaw_support_data = 0;
  }
  tele_data = data.tele - 2000;
